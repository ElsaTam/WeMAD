            ...
            <div class="col">
                <div class="card h-100 border-3 border-start-0 border-top-0 border-end-0 rounded-0" style="width: 250px; background-color: rgb(230, 229, 240);">
                    <div class="card-header text-center">
                        <a class="stretched-link link-dark text-decoration-none" href="{{ url('offices/'.$office->place_id) }}">
                            <h5>{{ $office->state->id }}, {{ $office->name }}</h5>
                        </a>
                    </div>
                    <img src="{{ URL::asset($office->boss->featuredPhoto ? $office->boss->featuredPhoto->src : '/storage/pictures/profile-unknown.png') }}" alt="">
                    <div class="card-body p-1">
                        <p class="card-text text-center">
                            <span class="fw-bold">{{ $office->boss->sex == "Homme" ? "Directeur" : "Directrice" }} :</span> {{ $office->boss->first_name }} {{ $office->boss->last_name }}
                            <br>
